...
public class BankApplication {//실행클래스
    ...
    //클래스 멤버
    //1. 필드
    //여러개의 account객체를 저장하는 배열
    static Account[] accounts=new Account[100];
    //main()함수가 아닌 main() 밖에 정적필드로 선언하는 이유 : 여럿 함수에서 사용할려고.
    ...
    static void 계좌생성(){
        ...
        Account account=new Account(계좌번호, 계좌주, 초기입금액);
            //2. 빈생성자 사용했을때 [권장하지 않은 -> 생성자 또는 메소드를 통한 필드 사용]
    ...
    static void 예금(){
        ...
        //*입력받은 계좌번호가 계좌목록에 존재하면 입력한 예금액을 찾은 계좌객체에서 예금에 더한다.[1명]
        ...
        for(int i=0; i<accounts.length; i++){
            Account account=accounts[i];
            if(account==null) break;
            //만약에 동일한 계좌번호의 객체가 있으면 해당 객체 내 입금액을 더한다.
    ...
    static void 출금() {
        ...
        String 계좌번호 = scanner.next();
        System.out.print("출금) 예금액 : ");
        int 출금액 = scanner.nextInt();
        //2. 계좌목록/배열 에서 입력한 계좌 찾는다.
        for (int i = 0; i < accounts.length; i++) {//for start
            Account account = accounts[i];
            if (account == null) break;
            //만약에 동일한 계좌번호의 객체가 있으면 해당 객체 내 입금액을 더한다.
